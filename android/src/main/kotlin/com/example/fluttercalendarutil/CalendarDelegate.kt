package com.example.fluttercalendarutil
...
import com.example.fluttercalendarutil.common.Constants.Companion.CALENDAR_PROJECTION
import com.example.fluttercalendarutil.common.Constants.Companion.CALENDAR_PROJECTION_ACCESS_LEVEL_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION
import com.example.fluttercalendarutil.common.Constants.Companion.CALENDAR_PROJECTION_ACCOUNT_NAME_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.CALENDAR_PROJECTION_DISPLAY_NAME_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.CALENDAR_PROJECTION_ID_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.CALENDAR_PROJECTION_OWNER_ACCOUNT_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_DESCRIPTION_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_ID_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_TITLE_INDEX
import com.example.fluttercalendarutil.common.ErrorCodes.Companion.NOT_FOUND
import com.example.fluttercalendarutil.common.ErrorCodes.Companion.INVALID_ARGUMENT
import com.example.fluttercalendarutil.models.Calendar
import com.example.fluttercalendarutil.models.Event
import io.flutter.plugin.common.MethodChannel
import com.google.gson.Gson
import android.provider.CalendarContract.Events
import com.example.fluttercalendarutil.common.Constants.Companion.ATTENDEE_EMAIL_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.ATTENDEE_EVENT_ID_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.ATTENDEE_ID_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.ATTENDEE_NAME_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.ATTENDEE_PROJECTION
import com.example.fluttercalendarutil.common.Constants.Companion.ATTENDEE_TYPE_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_ALL_DAY_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_BEGIN_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_DURATION_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_END_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_EVENT_LOCATION_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_RECURRING_DATE_INDEX
import com.example.fluttercalendarutil.common.Constants.Companion.EVENT_PROJECTION_RECURRING_RULE_INDEX
import com.example.fluttercalendarutil.common.ErrorCodes.Companion.GENERIC_ERROR
import com.example.fluttercalendarutil.common.ErrorCodes.Companion.NOT_ALLOWED
import com.example.fluttercalendarutil.common.ErrorCodes.Companion.NOT_AUTHORIZED
import com.example.fluttercalendarutil.common.ErrorMessages
import com.example.fluttercalendarutil.common.ErrorMessages.Companion.CALENDAR_ID_INVALID_ARGUMENT_NOT_A_NUMBER_MESSAGE
import com.example.fluttercalendarutil.common.ErrorMessages.Companion.CREATE_EVENT_ARGUMENTS_NOT_VALID_MESSAGE
import com.example.fluttercalendarutil.common.ErrorMessages.Companion.DELETING_RECURRING_EVENT_NOT_SUPPORTED_MESSAGE
import com.example.fluttercalendarutil.common.ErrorMessages.Companion.NOT_AUTHORIZED_MESSAGE
import com.example.fluttercalendarutil.models.Attendee
import com.example.fluttercalendarutil.models.CalendarMethodsParametersCacheModel
import java.util.*
...
public class CalendarDelegate : PluginRegistry.RequestPermissionsResultListener {
    ...
    public override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray): Boolean {
        ...
        when (cachedValues.calendarDelegateMethodCode) {
            ...
            RETRIEVE_CALENDAR_ID_METHOD_CODE -> {
                if (permissionGranted) {
                    retrieveCalendarId(cachedValues.calendarName, cachedValues.pendingChannelResult)
                } else {
                    finishWithError(NOT_AUTHORIZED, NOT_AUTHORIZED_MESSAGE, cachedValues.pendingChannelResult)
                }
    ...
    public fun retrieveCalendars(pendingChannelResult: MethodChannel.Result, isInternalCall: Boolean = false): MutableList<Calendar>? {
    ...
    @SuppressLint("MissingPermission")
    public fun retrieveCalendarId(calendarName: String, pendingChannelResult: MethodChannel.Result, isInternalCall: Boolean = false): String? {
        if (isInternalCall || arePermissionsGranted()) {
            val calendars = retrieveCalendars(pendingChannelResult, true);
            var foundCalendar = false

            if (calendars != null) {
                for (calendar: Calendar in calendars) {
                    if (calendar.name.equals(calendarName)) {
                        foundCalendar = true

                        if (isInternalCall) {
                            return calendar.id;
                        } else {
                            finishWithSuccess(calendar.id, pendingChannelResult);
                        }
                    }
                }
            }

            if (!isInternalCall && !foundCalendar) {
                finishWithError(NOT_FOUND, "The calendar with the name $calendarName could not be found", pendingChannelResult)
            }
        } else {
            val parameters = CalendarMethodsParametersCacheModel(pendingChannelResult, RETRIEVE_CALENDAR_ID_METHOD_CODE, calendarName)
            requestPermissions(parameters)
        }
        return null;
    }
    ...
    @SuppressLint("MissingPermission")
    public fun createCalendar(calendarName: String, pendingChannelResult: MethodChannel.Result) {
        if (arePermissionsGranted()) {
            // don't create if it already exists
            val existingCalId = retrieveCalendarId(calendarName, pendingChannelResult, true)
            if (existingCalId != null) {
                finishWithSuccess(existingCalId, pendingChannelResult);
                return
            }
            ...
            try {
                val created = contentResolver?.insert(calUri, values)
                var calendarId = java.lang.Long.parseLong(created?.getLastPathSegment())
