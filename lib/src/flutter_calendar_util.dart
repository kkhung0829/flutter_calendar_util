...
class FlutterCalendarUtil {
  static const MethodChannel channel =
      const MethodChannel('flutter_calendar_util');

  static final FlutterCalendarUtil _instance =
      new FlutterCalendarUtil._createInstance();

  factory FlutterCalendarUtil() {
    return _instance;
  }

  @visibleForTesting
  FlutterCalendarUtil._createInstance();
  ...
  /// Retrieve calendar id for calendar with name
  /// 
  /// The `calendarName` paramter is the name of the calendar that plugin will return id for
  /// 
  /// Returns a [Result] containing calendar id
  Future<Result<String>> retrieveCalendarId(String calendarName) async {
    final res = new Result<String>();
    ...
    try {
      res.data = await channel.invokeMethod('retrieveCalendarId',
  ...
  Future<Result<String>> createCalendar(String calendarName) async {
    ...
    try {
      res.data = await channel.invokeMethod('createCalendar',
        <String, Object>{'calendarName': calendarName});
    } catch (e) {
      _parsePlatformExceptionAndUpdateResult<String>(e, res);
    }
